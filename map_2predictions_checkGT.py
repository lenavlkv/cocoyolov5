...
def main(img_id):
    ...
    gt_used = [False] * len(anns)  # Массив для отслеживания использованных ground truth
    ...
    for i, pred in enumerate(results):
        ...
        for idx, (gt_box, gt_category) in enumerate(iou_results['gt']):
            if gt_used[idx]:  # Пропускаем уже использованные ground truth
                continue
        ...
        if best_gt_idx != -1 and max_iou >= 0.5:
            if label == best_gt:
                iou_results[label].append(1)  # True positive
                gt_used[best_gt_idx] = True  # Помечаем ground truth как использованный
            else:
