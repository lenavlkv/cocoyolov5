...
class MAP:
    ...
    def preprocessData(self):
        ...
        for filename in finalGTFiles:
            ...
            if anns.shape[0] == 0:
                ...
                anns = anns if anns.ndim > 1 else anns.reshape(1, -1)
                coco_to_voc = {v: k for k, v in voc_idx_to_coco_idx.items()}
                for det in anns:
                    coco_label = int(det[0])
                    if coco_label in coco_to_voc:
                        det[0] = coco_to_voc[coco_label]
    ...
    def calAPClass(self, gtBoxes, detBoxes, detScores, IoU=0.5):
        ...
        # # if there are no detections for the class
        # if len(scoresList) == 0:
        #     return {
        #         'AP': 0.0,
        #         'Precision': 0.0,
...
if __name__ == '__main__':
    ...
    parser.add_argument("-p", "--points", type=int, default=101,
                        help='Interpolation value: 0: Continues / 11: PascalVOC2012 Challenge')
    parser.add_argument("-n", "--names", default="pascal.names",
...
# python pascal_yolo5.py -g "C:\Users\Admin\PycharmProjects\cocoyolov5\VOCdevkit\VOC2012\labels_tlbr" -d "C:\Users\Admin\PycharmProjects\cocoyolov5\VOCdevkit\VOC2012\predictions_pascal" -i 0.5 -n pascal.names -p 101 -c 0.5

# from https://github.com/shreejalt/mAP-calculator/tree/main
# map = 0.716 (without topk)